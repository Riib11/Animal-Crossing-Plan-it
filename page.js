$(function() {
    ...
    function triggerRefresh() {
        if (goingToRefresh) {
            clearTimeout(goingToRefresh);
        }
        goingToRefresh = setTimeout(function() {
            ACNL.qr($("#qr"));
        }, 3000);
    }
            ...
            r.onload = function(re) {
                qrcode.callback = function(r) {
                    ...
                    if (r.length == 0x21C) {
                        //multi-part pattern
                        multi_icon += r;
                        if (multi_icon.length == 0x21C) {
                            alert("Please input the second code next.");
                            return;
                        }
                        if (multi_icon.length == 0x438) {
                            alert("Please input the third code next.");
                            return;
                        }
                        if (multi_icon.length == 0x654) {
                            alert("Please input the last code next.");
                            return;
                        }
                        if (multi_icon.length == 0x870) {
                            newIcon(multi_icon);
                            multi_icon = "";
                            return;
                        }
    ...
    function palette_top(imgdata) {
        ...
        for (var i = 0; i < 4096; i += 4) {
            myPal(imgdata.data[i], imgdata.data[i + 1], imgdata.data[i + 2]);
        }
    ...
    function palette_lowest(imgdata) {
        ...
        function myPal(pixel, r, g, b) {
            ...
            for (var i = 0; i < 256; i++) {
                ...
                if (match < 120) {
                    //this way, we get 1 or more matches, guaranteed
            ...
            r.onload = function(re) {
                ...
                img.onload = function() {
                    ...
                    case "top":
                        ...
                        break;
                    case "lowest":
                        palette_lowest(imgdata);
                        break;
                    case "grey":
                        palette_grey(imgdata);
                        break;
                    case "sepia":
                        palette_sepia(imgdata);
                        break;
                    }
                    reColorize(imgdata);
                    triggerRefresh();
