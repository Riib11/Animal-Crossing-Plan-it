...
(function ($) {
    ...
    createQr = function (typeNumber, correctLevel, text, multipart_num, multipart_total, multipart_parity) {

			// `qrcode` is the single public function that will be defined by the `QR Code Generator`
			// at the end of the file.
      var qr = qrencode(typeNumber, correctLevel, multipart_num, multipart_total, multipart_parity);
			qr.addData(text);
			qr.make();
    ...
    createBestQr = function (text, multipart_num, multipart_total, multipart_parity) {
		...
		createCanvas = function (settings) {
			...
			var $canvas = $('<canvas/>').attr('width', settings.width).attr('height', settings.height).css("margin-bottom", "40px");
